    ...
    <div className="space-y-6">
                  ...
                  <div className="bg-muted/30 rounded-lg p-4 space-y-4">
                    ...
                    {formData.content_type === 'password' && (
                      <div className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            <Label htmlFor="password-email">{t("emailOrUsername")}</Label>
                            <Input
                              id="password-email"
                              type="text"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div className="space-y-2">
                            <Label htmlFor="password-website">{t("websiteOrApp")}</Label>
                            <Input
                              id="password-website"
                              type="url"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredNotes.map(note => {
          ...
          return (
            <Card key={note.id} className="hover:shadow-lg transition-shadow min-w-0 overflow-hidden">
                ...
                <div className="flex items-start justify-between mb-3 min-w-0">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 min-w-0">
                      ...
                      <h3 className="font-semibold line-clamp-1 break-all">{note.name}</h3>
                    </div>
                    {note.description && (
                      <p className="text-sm text-muted-foreground line-clamp-2 break-all">{note.description}</p>
                    )}
                  </div>
                </div>
{/* --------------------------------------------------------------------------------------------------------------------- */}
                ...
                {note.content_type === 'password' && note.content?.password && (
                  <div className="bg-muted/50 rounded-md p-3 mb-3 space-y-2 min-w-0 overflow-hidden">
                    <div className="flex items-center justify-between min-w-0">
                      <span className="text-sm font-semibold shrink-0">🔐 Tài khoản</span>
                    ...
                    <div className="text-sm space-y-1.5 min-w-0 break-all overflow-hidden">
                      {note.content.password.email && (
                        <div className="break-all flex flex-wrap items-baseline gap-1">
                          <span className="text-muted-foreground shrink-0">{t("email")}:</span>
                          <span className="break-all select-all font-medium">{note.content.password.email}</span>
                        </div>
                      )}
                      {note.content.password.username && (
                        <div className="break-all flex flex-wrap items-baseline gap-1">
                          <span className="text-muted-foreground shrink-0">{t("username")}:</span>
                          <span className="break-all select-all font-medium">{note.content.password.username}</span>
                        </div>
                      )}
                      {note.content.password.website && (
                        <div className="break-all flex flex-wrap items-baseline gap-1">
                          <span className="text-muted-foreground shrink-0">{t("website")}:</span>
                          <a 
                            href={note.content.password.website.startsWith('http') ? note.content.password.website : `https://${note.content.password.website}`}
                            target="_blank" 
                            rel="noopener noreferrer"
                            className="break-all text-primary hover:underline font-medium select-all"
                          >
                            {note.content.password.website}
                          </a>
                        </div>
                      ...
                      <div className="break-all flex flex-wrap items-baseline gap-1">
                        <span className="text-muted-foreground shrink-0">{t("password")}:</span>{' '}
                ...
                {contentText && note.content_type !== 'password' && note.content_type !== 'image' && (
                  <div className="bg-muted/30 rounded-md p-3 mb-3 relative overflow-hidden min-w-0">
                    <div
                      className={`text-sm ${note.content_type === 'code' ? 'font-mono' : ''} whitespace-pre-wrap overflow-hidden transition-all duration-200 break-all`}
                      style={{
